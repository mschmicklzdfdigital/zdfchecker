...
SITEMAPS = [
    "https://www.zdf.de/sitemap.xml",
    "https://www.zdf.de/sitemap-index.xml",
...
def scrape_sitemap(sitemap_url: str, start_date: date, end_date: date) -> list[dict]:
    ...
    for url_tag in root.findall(".//sm:url", ns):
        ...
        # Filter to ZDF domains
        if not any(domain in loc for domain in ["zdf.de", "heute.de"]):
            continue
...
def scrape_zdf_homepage_and_categories(start_date: date, end_date: date) -> list[dict]:
    """Scrape ZDF homepage and category pages for article links."""
    articles = []
    visited = set()

    category_pages = [
        "https://www.zdf.de/nachrichten/heute-sendung",
        "https://www.zdf.de/nachrichten",
        "https://www.zdf.de/nachrichten/politik",
        "https://www.zdf.de/nachrichten/wirtschaft",
        "https://www.zdf.de/nachrichten/gesellschaft",
        "https://www.zdf.de/nachrichten/panorama",
        "https://www.zdf.de/nachrichten/sport",
        "https://www.zdf.de/nachrichten/wissen",
        "https://www.heute.de",
    ]

    article_url_pattern = re.compile(
        r"https?://(?:www\.)?(?:zdf\.de|heute\.de)/(?:nachrichten|sport|kultur|serien-und-filme)[^\"'\s>]+"
    )
    ...
    for page_url in category_pages:
        ...
        for a_tag in soup.find_all("a", href=True):
            ...
            if not href.startswith("http"):
                href = urljoin("https://www.zdf.de", href)

            if not any(domain in href for domain in ["zdf.de", "heute.de"]):
                continue

            # Only article-like URLs
            if not re.search(r"/\d{3,}", href) and "/nachrichten/" not in href:
                continue
            ...
            # Try to get date from URL (e.g. /2024/01/15/)
...
def main():
    ...
    def render_tab(df: pd.DataFrame, published: bool):
        if df.empty:
        ...
        with filter_col1:
            ...
            selected_cat = st.selectbox("🏷️ Kategorie filtern", cats, key=f"cat_{published}")
        with filter_col2:
            search_q = st.text_input("🔍 Suche in Titeln/URLs", placeholder="Suchwort eingeben...", key=f"search_{published}")
    ...
    with tab1:
        render_tab(not_published_df, published=False)

    with tab2:
        render_tab(published_df, published=True)

    with tab3:
        render_tab(zdf_df, published=False)
